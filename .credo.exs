    ...
    %{
      ...
      checks: %{
        enabled: [
          ...
          #
          # The codebase deliberately uses fully-qualified module names
          # (e.g. `Localize.Currency`, `Money.Currency.Store`). Only flag a
          # module that is referenced often enough that an alias clearly pays
          ...
          #
          # Disabled: the `apply/3` sites in this codebase are deliberate
          # dispatch to optional dependencies (`DigitalToken`) and to
          # runtime-configured modules (callback/api/cache modules). A direct
          # call would defeat the optional-dependency guarding, and a test uses
          # `apply/3` specifically to sidestep the set-theoretic type checker.
