  ...
  * `{:error, {Money.ExchangeRateError, reason}}` — the request failed
  ...
  defp init_cache do
    with :undefined <- :ets.whereis(:etag_cache),
         do: :ets.new(:etag_cache, [:named_table])
  end
  ...
  defp save_headers(cache, url, headers) do
    ...
    :ets.insert_new(cache, {url, nil, nil})
    if etag, do: :ets.update_element(cache, url, {2, etag})
    if last_modified, do: :ets.update_element(cache, url, {3, last_modified})
  end
end
